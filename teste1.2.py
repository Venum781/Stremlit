...
# Calcular a taxa de crescimento/queda (variação percentual) para escolas municipais
...
# Calcular a taxa de crescimento/queda (variação percentual) para matrículas
df_matriculas["Variação (%)"] = df_matriculas["Matrículas"].pct_change() * 100
df_matriculas["Variação (%)"] = df_matriculas["Variação (%)"].fillna(0).round(2)  # Preencher NA com 0 e arredondar
...
def plot_main_data(selected_data):
    ...
    bars = ax.bar(selected_data["Ano"], selected_data["Escolas Municipais"], color='#4CAF50', zorder=3)
...
def plot_variation_data(selected_data):
    ...
    ax1.plot(selected_data["Ano"], selected_data["Variação (%)"], marker='o', color='#FF5733', label="Taxa de Queda", zorder=3)
    ax1.set_xlabel("Ano", fontsize=14, labelpad=10)
    ax1.set_ylabel("Taxa de Variação (%)", fontsize=14, labelpad=10, color='#FF5733')
...
def plot_enrollment_data(selected_data):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    bars = ax.barh(selected_data["Ano"], selected_data["Matrículas"], color='#FFC300', zorder=3)
...
def plot_enrollment_variation_data(selected_data):
    """Exibir o gráfico de taxa de variação das matrículas com valores sobrepostos."""
    fig, ax1 = plt.subplots(figsize=(10, 6))
    ...
    for i, val in enumerate(selected_data["Variação (%)"]):
        y_pos = val
        if i > 0:  # Apenas para anos subsequentes a 2019
            ax1.text(selected_data["Ano"][i], y_pos, f'{val}%\n{int(selected_data["Matrículas"].iloc[i - 1] - selected_data["Matrículas"].iloc[i])}',
                     ha='center', fontsize=12, color='red', linespacing=1.5)  # Sobrepor porcentagem e número inteiro em vermelho
        else:
            ax1.text(selected_data["Ano"][i], y_pos, f'{val}%', ha='center', fontsize=12, color='red')

    ax1.set_title("Taxa de Queda nas Ofertas de Matrículas", fontsize=16, pad=20)
    plot_common_style(ax1)
    st.pyplot(fig)

# Adicionar estilo CSS para melhorar a aparência da interface
st.markdown("""
    <style>
    body {
...
if menu_option == "Escolas Municipais Fechadas":
    ...
    # Menu de seleção de gráfico para matrículas
    option = st.selectbox("Escolha a visualização:", ["Ofertas de Matrículas", "Taxa de Queda nas Matrículas", "Ambos os Gráficos"])

    # Exibir os gráficos de acordo com a escolha do usuário
    if option == "Ofertas de Matrículas":
        st.subheader("Gráfico das Ofertas de Matrículas")
        plot_enrollment_data(df_matriculas)
    elif option == "Taxa de Queda nas Matrículas":
        st.subheader("Gráfico da Taxa de Queda nas Matrículas")
        plot_enrollment_variation_data(df_matriculas)
    elif option == "Ambos os Gráficos":
        st.subheader("Gráfico das Ofertas de Matrículas")
        plot_enrollment_data(df_matriculas)
        st.subheader("Gráfico da Taxa de Queda nas Matrículas")
